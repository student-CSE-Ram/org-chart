...
interface EmployeeDetailsDrawerProps {
  ...
  onDeleteEmployee?: (employeeCode: string) => void;
  userRole?: string;
  isDarkMode?: boolean;
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/35 backdrop-blur-xs animate-fadeIn">
      ...
      >
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap min-w-0">
            ...
            <span
              className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase ${
                employee.status?.toLowerCase() === 'vacant' || employee.name?.toLowerCase().includes('vacant')
        ...
        <div className="p-4 sm:p-6 overflow-y-auto flex-1 flex flex-col gap-5 sm:gap-6">
          {(employee.status?.toLowerCase() === 'vacant' || employee.name?.toLowerCase().includes('vacant')) && (
            <div className="p-3 rounded-xl bg-amber-500/15 border border-amber-500/30 text-amber-900 dark:text-amber-300 text-xs font-semibold flex items-center gap-2">
              <span className="w-2 h-2 rounded-full bg-amber-500 animate-ping flex-shrink-0" />
              <span>Vacant Position — This role in {employee.department} is currently unfilled and open for hiring.</span>
            </div>
          )}
        ...
        >
          {isAuthorizedRole ? (
            <div className="flex items-center gap-2">
              <button
                onClick={() => {
                  if (onEditEmployee) onEditEmployee(employee);
                }}
                ...
                title="Edit Employee details or change reporting manager"
              >
              ...
              <button
                onClick={() => {
                  if (
                    onDeleteEmployee &&
                    confirm(
                      `Are you sure you want to remove or mark ${employee.name} as Vacant? If they have direct reportees, their position will be converted to Vacant so direct reports remain intact.`
                    )
                  ) {
                    onDeleteEmployee(employee.employeeCode);
                    onClose();
                  }
                }}
                className="px-3 py-2 rounded-xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border border-rose-500/20 text-xs font-semibold flex items-center gap-1.5 transition-all"
                title="Delete employee or convert position to Vacant if reports exist"
              >
                <Trash2 className="w-3.5 h-3.5" />
                <span>Remove / Vacant</span>
              </button>
            </div>
          ) : (
