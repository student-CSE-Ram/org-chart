...
export const ExportModal: React.FC<ExportModalProps> = ({
  ...
  const [selectedScope, setSelectedScope] = useState<string>('Executive Board');
  ...
  const handleExport = async (type: 'png' | 'jpeg' | 'pdf' | 'svg' | 'print' | 'html') => {
    ...
    try {
      if (type === 'png' || type === 'jpeg' || type === 'svg') {
        ...
        printCompleteOrgChart(employees, branding, selectedScope);
      } else if (type === 'html') {
        downloadStandaloneHtmlBundle(
          employees,
          branding,
          selectedScope,
          `${branding.companyName || 'Gallantt'}_OrgChart.html`
        );
      }
    } catch (err) {
      console.error('Export failed:', err);
      setErrorMessage(
        `Export failed: ${(err as Error).message || 'Export error'}. Try another format or scope.`
      );
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/70 backdrop-blur-sm animate-fadeIn">
      <div className="w-full max-w-lg max-h-[90vh] sm:max-h-[85vh] bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden text-slate-900 dark:text-slate-100">
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-50 dark:bg-slate-900/90">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="text-sm sm:text-base font-extrabold truncate">Export Org Chart Document</h3>
              <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 truncate">
                Select target scope & format ({employees.length} Members Total)
              </p>
            </div>
          </div>
          <button
        ...
        {/* Department / Scope Selector Banner */}
        <div className="mx-4 sm:mx-6 mt-3.5 p-3.5 bg-blue-50 dark:bg-slate-800/90 border border-blue-200 dark:border-slate-700/80 rounded-xl flex flex-col gap-2">
          <label className="text-[11px] font-extrabold text-blue-900 dark:text-blue-200 uppercase tracking-wider flex items-center justify-between">
            <span>Target Scope / Department to Export</span>
            <span className="text-[10px] font-bold text-blue-600 dark:text-blue-400">
              {selectedScope === 'FULL_COMPANY' ? 'Full Organization' : selectedScope}
            </span>
          </label>
          <select
            value={selectedScope}
            onChange={(e) => setSelectedScope(e.target.value)}
            className="w-full text-xs font-bold p-2.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 text-slate-800 dark:text-slate-100 shadow-2xs focus:ring-2 focus:ring-blue-500 focus:outline-none"
          >
            <option value="Executive Board">👑 Executive Board (CMD & 4 Directors / CEO)</option>
            {departmentList.map((dept) => (
              <option key={dept} value={dept}>
                🏢 {dept} Department ({employees.filter((e) => e.department === dept).length} Members)
              </option>
            ))}
            <option value="FULL_COMPANY">🌐 Full Organization (All Departments)</option>
          </select>
          <p className="text-[10.5px] text-slate-600 dark:text-slate-400 font-medium leading-snug">
            Select a specific department above to export a clean, high-resolution single file dedicated to that team.
          </p>
        </div>

        {/* Error Alert Banner */}
        {errorMessage && (
          <div className="mx-4 sm:mx-6 mt-3 p-3 rounded-xl bg-rose-500/10 border border-rose-500/30 text-rose-600 dark:text-rose-300 text-xs font-semibold">
            {errorMessage}
          </div>
        )}
