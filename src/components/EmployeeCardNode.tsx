...
import { User, ChevronDown, ChevronRight, Building2, MapPin, Crown, Zap, Wrench } from 'lucide-react';
...
export const EmployeeCardNode = memo(({ data }: { data: EmployeeNodeData }) => {
  ...
  return (
    ...
    >
      ...
      <div className={`h-1.5 w-full ${isShiftIncharge ? 'bg-gradient-to-r from-violet-600 to-indigo-600' : locationTheme.accentBgClass}`} title={`Site: ${employee.location || 'Default'}`} />
      ...
      <div className={`py-1.5 px-3.5 w-full flex items-center justify-between border-b ${
        isShiftIncharge ? 'bg-violet-50 border-violet-100 text-violet-900' : 'bg-slate-50 border-slate-100 text-slate-600'
      }`}>
      ...
      <div className="p-3.5 flex flex-col gap-2.5 bg-white">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="min-w-0 flex-1">
            <h4 className="text-sm font-semibold text-slate-900 tracking-tight leading-snug truncate">
              {employee.name}
            </h4>
            ...
            <div className="flex items-center gap-1.5 mt-1">
              <span className="inline-block text-[10px] font-mono font-medium px-1.5 py-0.5 rounded-md bg-slate-100 text-slate-700 border border-slate-200">
